...
- [x] Spectrum from both radios — the SDR sweeps sub-2 GHz, and the Wi-Fi adapter reports the driver's own per-channel busy time and noise across 2.4 and 5 GHz, which the SDR cannot tune at all. Not an FFT and not claimed to be one: occupancy, published on the heartbeat, costing no radio time because it is a by-product of listening
